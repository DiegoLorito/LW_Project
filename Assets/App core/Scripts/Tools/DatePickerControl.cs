...
public class DatePickerControl : MonoBehaviour {
	...
	public void InputDateTime(){
		...
		else if (dateOn)
		{
			...
			int minYear = curYear - 15;

			if (y >= minYear && y <= curYear)
			{
				try
				{
	...
	public void yearMin(){
		...
		if(fecha.Year - 1 >= minYear)
        {
			try
			{
				if (fecha.Year - 1 >= minYear) fecha = new DateTime(fecha.Year - 1, fecha.Month, fecha.Day, fecha.Hour, fecha.Minute, fecha.Second);
			}
			catch
			{
				if (fecha.Year > minYear)
				{
					try
					{
						//diaMin ();
